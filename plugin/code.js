...
function handleBuildMessage(msg) {
  ...
  return Promise.resolve(runBuild(spec))
    .then(function (result) {
      if (!result) {
        throw new Error('Build returned no result');
      }
      var sections = Array.isArray(result.sections) ? result.sections : [];
      var page = result.page;
      var rootFrame = result.rootFrame;
      if (!page || !rootFrame) {
        throw new Error('Build returned no page/frame (internal error)');
      }
    ...
    .catch(function (error) {
      ...
      var payload = { type: 'build:error', message: message };
      if (error && typeof error.stack === 'string') {
        payload.stack = error.stack;
      }
      if (error && Array.isArray(error.logs)) {
        payload.logs = error.logs;
      }
