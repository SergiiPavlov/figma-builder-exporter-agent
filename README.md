...
- При сравнении экспортированных фреймов агент проверяет ключевые метрики авто-лейаута: `paddingTop/Right/Bottom/Left`, `itemSpacing`, `gridGap` и `layoutMode`.
- Порог чувствительности берётся из `TaskSpec.acceptance.maxSpacingDeviation` (если поле не задано — используется дефолт `2px`).
- Отклонения с модулем `delta` меньше либо равным `tolerancePx` не попадают в итоговый отчёт.
